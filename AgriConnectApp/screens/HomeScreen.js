...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Tải dữ liệu (sau khi seed hoặc nếu đã có)
  ...
  // Các lựa chọn mùa (có Tất cả)
  const seasons = ["Tất cả mùa", "Xuân", "Hạ", "Thu", "Đông"];

  // Lọc sản phẩm theo mùa và tìm kiếm:
  ...
  const renderProduct = ({ item }) => (
    ...
    >
      ...
      <Text style={styles.productMeta}>
        {item.season ? `Mùa: ${item.season}` : ""} {item.location ? `• ${item.location}` : ""}
      </Text>
    </TouchableOpacity>
  );
