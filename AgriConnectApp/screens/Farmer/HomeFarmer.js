    ...
    const unsub = firestore()
      ...
      .onSnapshot((snapshot) => {
        ...
        snapshot.forEach((doc) => {
          const data = doc.data();
          totalUnread += data.unreadCount?.[uid] || 0;
        });
  ...
  useEffect(() => {
    ...
    const productsUnsub = firestore()
      ...
      .onSnapshot((snapshot) => {
        ...
        setProducts(list);
        const totalOrders = list.reduce((sum, p) => sum + (p.ordersCount || 0), 0);
        setTotalSold(totalOrders);
      });

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const ordersUnsub = firestore()
      .collection("orders")
      .where("sellerId", "==", uid)
      .onSnapshot((snapshot) => {
        let todayCount = 0;
    ...
    >
      ...
      <View style={styles.welcomeBanner}>
        <View style={styles.bannerContent}>
          <Image
            source={{ uri: farmerData?.photoBase64 || farmerData?.photoURL || DEFAULT_AVATAR }}
      ...
      <View style={styles.quickActions}>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "#27ae60" }]} onPress={() => navigation.navigate("AddProduct")}>
          <Icon name="add-circle" size={32} color="#fff" />
          <Text style={styles.actionText}>Thêm sản phẩm</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "#3498db" }]} onPress={() => navigation.navigate("OrderManagement")}>
          <Icon name="basket" size={32} color="#fff" />
          <Text style={styles.actionText}>Đơn hàng</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "#9b59b6" }]} onPress={() => navigation.navigate("Chat")}>
      ...
      {products.length === 0 ? (
        <View style={styles.emptyProducts}>
          <Image source={{ uri: "https://cdn-icons-png.flaticon.com/512/7484/7481385.png" }} style={{ width: 100, height: 100, opacity: 0.6 }} />
          <Text style={styles.emptyText}>Chưa có sản phẩm nào</Text>
          <TouchableOpacity style={styles.addFirstBtn} onPress={() => navigation.navigate("AddProduct")}>
        ...
        <View style={styles.productsGrid}>
          {products.map((item) => {
            const stockInfo = getStockStatus(item.stock || item.quantityAvailable || 0);
            return (
              <TouchableOpacity
                key={item.id}
              ...
              >
                <Image
                  source={{ uri: item.imageBase64 || item.imageUrl || "https://via.placeholder.com/150" }}
                ...
                <View style={styles.productContent}>
                  ...
                  <Text style={styles.location}>{item.region || item.growingRegion || "Việt Nam"}</Text>
                </View>
              </TouchableOpacity>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  welcomeBanner: { backgroundColor: "#27ae60", marginHorizontal: 20, marginTop: 40, marginBottom: 16, padding: 20, borderRadius: 24, elevation: 10 },
  ...
  actionBtn: { alignItems: "center", padding: 16, borderRadius: 20, width: "30%", elevation: 6 },
  actionText: { color: "#fff", marginTop: 8, fontWeight: "600" },
  statsContainer: { flexDirection: "row", marginHorizontal: 20, marginBottom: 16, backgroundColor: "#fff", borderRadius: 20, paddingVertical: 20, elevation: 6 },
  statBox: { flex: 1, alignItems: "center" },
  statNumber: { fontSize: 22, fontWeight: "bold", color: "#2c3e50", marginTop: 8 },
  statLabel: { fontSize: 13, color: "#7f8c8d", marginTop: 4 },
  tipCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff9e6", marginHorizontal: 20, marginBottom: 16, padding: 16, borderRadius: 16, elevation: 4 },
  ...
  productsGrid: { paddingHorizontal: 16, flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginBottom: 20 },
  productCard: { width: "48%", backgroundColor: "#fff", borderRadius: 20, marginBottom: 16, elevation: 6, overflow: "hidden" },
  productImage: { width: "100%", height: 140 },
  stockBadge: { position: "absolute", top: 10, left: 10, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20 },
  ...
  chatBadge: { position: "absolute", top: -6, right: -10, backgroundColor: "#e74c3c", minWidth: 20, height: 20, borderRadius: 10, justifyContent: "center", alignItems: "center", paddingHorizontal: 6, borderWidth: 2, borderColor: "#fff" },
  chatBadgeText: { color: "#fff", fontSize: 11, fontWeight: "bold" },
});